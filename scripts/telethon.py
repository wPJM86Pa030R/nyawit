...
async def remove_command(event):
    ...
    if not path_text and event.is_reply:
        replied = await event.get_reply_message()
        path_text = (replied.message or "").strip()
...
async def copy_command(event):
    ...
    if not source_text or not destination_text:
        await safe_edit(event, "Format copy tidak valid.")
        return
    ...
    try:
        ...
        await safe_edit(event, f"Tujuan tidak valid: `{e}`")
        return
...
async def move_command(event):
    ...
    destination_text = args[-1].strip()
    if not source_text or not destination_text:
        await safe_edit(event, "Format move tidak valid.")
        return
